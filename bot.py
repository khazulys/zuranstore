...
def callback_inline(call: CallbackQuery):
    ...
    if action == 'product':
        ...
        if count > stok[product.replace('_',' ')]:
          markup = InlineKeyboardMarkup()
          markup.add(InlineKeyboardButton('Back to menu', callback_data='back'))
          bot.edit_message_text(f"Stok {product.replace('_',' ')} tidak cukup. Stok yang tersedia: {stok[product.replace('_',' ')]}", chat_id, message_id, reply_markup=markup)
        #bot.send_chat_action(chat_id, 'typing')
        #time.sleep(1)
        else:
          markup = InlineKeyboardMarkup(row_width=4)
